...
in
  {
    ...
    config = mkIf cfg.enable {
      ...
      # ETC_ZSHRC_DONT_LOAD_OH_MY_ZSH can be specified in case an individual user wants to load their own version
      # of oh-my-zsh (or something else entirely different) on a system where it's otherwise enabled globally, and
      # to avoid double-loading if they have it in their .zshrc
      programs.zsh.interactiveShellInit = ''
        # Load NixOS' oh-my-zsh
        # Can be disabled on a per-user basis by setting `ETC_ZSHRC_DONT_LOAD_OH_MY_ZSH=1` in ~/.zshenv
        if [[ "''${ETC_ZSHRC_DONT_LOAD_OH_MY_ZSH:-}" != "1" ]]; then
